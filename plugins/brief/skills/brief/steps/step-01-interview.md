...
description: Targeted clarifying questions, loop until enough signal to draft the brief
prev_step: steps/step-00-init.md
next_step: steps/step-02-draft-brief.md
---
...
- 🛑 NEVER draft the brief in this step
- 🛑 NEVER pick a technical solution (that is propose, not brief)
...
- Available from init: feature_name, feature_slug, brief_path, tasks_path, auto_mode, skip_interview
- If `skip_interview` = true, do not run this step; load step-02 directly
- Don't assume knowledge from future steps

## YOUR TASK:

Elicit the product signal needed to write the brief through targeted questions, looping until ambiguity is resolved.
...
questions:
  - header: "Scoping"
    question: "Enough signal to draft the brief?"
    options:
      - label: "Draft (Recommended)"
        description: "Scoping is sufficient, move to the brief"
      - label: "Re-question"
...
After the decision gate passes, load `./step-02-draft-brief.md`.
