...
const storage = join(homedir(), '.omp', 'agent', 'arsenal-mode');
const guidance = `

## Arsenal mode

Arsenal mode is enabled. For every request, read and follow skill://arsenal as
the workflow router. Let Arsenal select any installed specialist skill needed
for the request. Use the native OMP tools and permissions without additional
...
export default function arsenalMode(pi: ExtensionAPI) {
  ...
  pi.registerCommand('arsenal-mode', {
    description: 'Toggle continuous Arsenal routing without restricting OMP.',
    ...
    handler: async (args, ctx) => {
      ...
      enabled = mode === 'on';
      save();
      ctx.ui.notify(`Arsenal mode: ${mode}. Native OMP permissions unchanged.`, 'info');
    },
  });
}
