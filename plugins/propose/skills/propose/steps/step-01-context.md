...
- ✅ ALWAYS write findings to section 2 of PROPOSAL.md
...
- 💾 Append findings to PROPOSAL.md section 2 + state in frontmatter
- 📖 Complete context gathering before loading step-02
- 🚫 FORBIDDEN to load step-02 until section 2 is written

## CONTEXT BOUNDARIES:

- Variables from step-00: `{proposal_path}`, `{scope_path}`, `{auto_mode}`
- Output: section 2 filled + `context_collected` in frontmatter
- Resources: GitNexus MCP, Read, Grep, Glob

## YOUR TASK:

Gather objective codebase context (modules, symbols, prior art) relevant to the proposal scope. No interpretation, no design.
  ...
  - header: "Scope"
    ...
    options:
      - label: "Auto-detect from the title"
        description: "Extract keywords from the proposal title"
...
Look for:
- decision records (`docs/short/**`, `docs/decisions/**`)
- Previous proposals (`docs/proposals/**`)
- Related TODOs/FIXMEs in scope
- CHANGELOG entries touching the area

### 4. Write section 2 of PROPOSAL.md
...
✅ Section 2 of PROPOSAL.md filled with concrete refs
...
Proceed to `./step-02-problem.md`. Tell the user in one short line, in the conversation language, what context was gathered (e.g. "context: 4 modules, 2 prior proposals") - do NOT paste the section markdown into the chat. No "continue?" confirmation gate. The user may say "stop" or "enrich context" to loop back.
