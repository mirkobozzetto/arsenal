...
- 🛑 NEVER draw a diagram that restates a sentence: a diagram earns its place
  by showing a topology, a race or a lifecycle that prose cannot carry
...
- Variables: `{proposal_path}`, `{auto_mode}`, `alternatives_count`, `context_collected`
- Output: section 6 of PROPOSAL.md
...
✅ Data model covered, or the sentence "no data changes" and nothing more
✅ API contracts covered, or "no API changes" and nothing more
✅ GitNexus impact run on affected symbols
✅ HIGH/CRITICAL risks flagged
✅ In `short`: this whole section fits in a short paragraph plus the files

## FAILURE MODES:

❌ No diagram where the change is a topology or a lifecycle
❌ A diagram drawn to satisfy the template, restating the paragraph above it
